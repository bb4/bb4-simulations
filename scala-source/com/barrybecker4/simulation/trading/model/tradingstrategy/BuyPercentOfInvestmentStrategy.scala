...
class BuyPercentOfInvestmentStrategy extends TradingStrategy {
  ...
  override def getName = "percent of investment"
  ...
  override def updateInvestment(stockPrice: Double): MarketPosition = {
    ...
    if (pctChange >= gainPolicy.getChangePercent) { // sell, and take some profit. Assume we can sell partial shares
      val sharesToSell = Math.min(sharesOwned, gainPolicy.getTransactPercent * reserve / stockPrice)
      sell(sharesToSell, stockPrice)
    }
    else if (-pctChange >= lossPolicy.getChangePercent) { // buy more because its cheaper
      val amountToInvest = Math.min(reserve, lossPolicy.getTransactPercent * sharesOwned * stockPrice)
  ...
  override def getOptionsUI: JPanel = {
    ...
    gainPolicyPanel = new ChangePolicyPanel(
      "% gain which triggers next transaction", "% of current reserve to sell on gain", gainPolicy)
    lossPolicyPanel = new ChangePolicyPanel(
      "% market loss which triggers next transaction", "% of current investment to use to buy on loss", lossPolicy)
