...
class GraphingOptionsPanel() extends JPanel {
  ...
  private val xResolutionField =
    new NumberInput("Histogram Resolution (1 - 5): ", graphOptions.histogramXResolution,
      "1 is low resolution 5 is high (meaning more bins on the x axis).", 1, 5, true)
  ...
  useLogScale.setToolTipText("If checked, " + "the x axis will be shown on a log scale so that the histogram will be easier to interpret.")
  private var numRecentSeriesField =
    new NumberInput("Num recent series: ", graphOptions.numRecentSeries,
      "The number of recent time series to show in the stock generation and investment line " + "charts on the left", 1, 1000, true)
