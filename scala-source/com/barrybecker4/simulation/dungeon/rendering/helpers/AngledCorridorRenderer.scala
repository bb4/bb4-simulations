...
case class AngledCorridorRenderer(g: OfflineGraphics, options: DungeonOptions) {

  def drawCorridor(path: Seq[IntLocation], decoration: RoomDecoration): Unit = {
    val loc1 = path.head
    val loc2 = path(1)
    val loc3 = path(2)
    // there are 4 cases l T, T, reverse L

    drawSegment(loc1.getX,  loc1.getY, loc2.getX, loc2.getY, decoration)
    drawSegment(loc2.getX,  loc2.getY, loc3.getX, loc3.getY, decoration)
  }

  private def drawSegment(px: Int, py: Int, x: Int, y:Int, decoration: RoomDecoration): Unit = {
    val box =
