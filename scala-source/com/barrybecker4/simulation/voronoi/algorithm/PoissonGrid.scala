...
case class PoissonGrid(width: Double, height: Double, radius: Double, rnd: Random = RND) {
  private val cellSize = radius / Math.sqrt(2)
  ...
  def getSampleIndex(x: Double, y:Double): Int = {
    grid(getIdx(x))(getIdx(y))
  }

  def getPoint(index: Int): Point2d = {
    samples(index)
  }

  def addSample(point: Point2d): Int = {
    println("adding point to grid: " + point)
    samples :+= point
    grid(getIdx(point.x))(getIdx(point.y)) = samples.length - 1
    samples.length - 1
  }
  ...
  def addNewElementIfPossible(index: Int, k: Int): Int = {
    ...
    for (i <- 0 until k) {
      val nbrPoint = getNeighborOf(point)
  ...
  private def getNeighborOf(point: Point2d): Point2d = {
    var x: Double = -1
    var y: Double = -1
    var ct = 0
